...
class kitti_val(BaseDataset):
    def __init__(self, data_path, filenames_path='./dataset/filenames/',
        ...
        if self.args.kitti_validation_split:
            self.data_path =  self.args.data_path
            self.img_list = glob.glob(os.path.join(self.data_path,"image/*"))
            self.gt_list = glob.glob(os.path.join(self.data_path,"groundtruth_depth/*"))
            self.img_list.sort()
            self.gt_list.sort()
            phase = 'train' if is_train else 'test'
            print("# of %s images: %d" % (phase, len(self.img_list)))
        elif self.args.kitti_held_out_test_set:
            self.data_path =  self.args.data_path
            self.img_list = glob.glob(os.path.join(self.data_path,"image/*"))
            # self.gt_list = glob.glob(os.path.join(self.data_path,"groundtruth_depth/*"))
            self.img_list.sort()
    ...
    def __len__(self):
        if self.args.kitti_validation_split or self.args.kitti_held_out_test_set:
    ...
    def __getitem__(self, idx):
        ...
        if self.args.kitti_validation_split:
            img_path = os.path.join(self.img_list[idx])
            gt_path = os.path.join(self.gt_list[idx])
        ...
        if not self.args.kitti_held_out_test_set:
        ...
        if self.is_train:
            ...
            if not self.args.kitti_held_out_test_set:
                ...
                image, depth = self.augment_test_data(image, image)
        depth = depth / (256.0)
